...
class Sidebar extends React.PureComponent {
  ...
  transFormMenuItem(obj, paths, isLevel1) {
    let parentPath = paths.join('/');   // 将各级父目录组合成完整的路径
    parentPath = parentPath.replace(/\/{2,}/, '/').replace(/^\//, '');//将连着的/// 替换成一个单独的/ 并且去掉字符串头部的/
    ...
    return (
      <MenuItem key={obj.key} style={{ margin: '0px' }}>
        ...
        <Link  activeClassName="link-active"  to={`/${parentPath}`} style={{ display: 'inline' }}><span className="nav-text">{obj.name}</span></Link>
      </MenuItem>
    );
  }
  ...
  componentWillMount() {
    ...
    const menu = items.map((level1) => {
      ...
      // 是否有子菜单?
      if (level1.children) {
        const level2menu = level1.children.map((level2) => {
          // parse二级菜单
          paths.push(level2.key);
          level2KeyMap.set(level2.key, level1.key);

          if (level2.children) {
            const level3menu = level2.children.map((level3) => {
